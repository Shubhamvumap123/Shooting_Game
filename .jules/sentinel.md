...
## 2025-05-21 - Vercel Security Headers
**Vulnerability:** Missing standard security headers (X-Frame-Options, X-Content-Type-Options, etc.) on Vercel deployment.
**Learning:** `react-scripts` does not automatically configure server-side headers. For Vercel deployments, these must be explicitly defined in a `vercel.json` configuration file at the root.
**Prevention:** Ensure `vercel.json` is present and maintained with the latest recommended security headers for all Vercel-deployed projects.
