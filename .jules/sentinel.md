...
**Vulnerability:** Missing HTTP security headers (like `X-Frame-Options`, `X-Content-Type-Options`) exposed the application to clickjacking and MIME-type sniffing attacks.
**Learning:** Vercel deployments can be hardened by adding a `vercel.json` file with specific header configurations. This is a deployment-level configuration that complements application-level security (like CSP).
**Prevention:** Always verify deployment configuration files (like `vercel.json` or `netlify.toml`) include standard security headers to implement Defense in Depth.
