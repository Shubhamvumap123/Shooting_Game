...
function App() {
  ...
  function drawStars() {
    ...
    ctx.fillStyle = "white"; // Performance: Set static color once

    // Performance: Cache time to avoid Date.now() syscalls in loop
    const now = Date.now();

    stars.current.forEach(star => {
      ctx.beginPath();
      // Performance: Use globalAlpha instead of parsing rgba string
