...
function App() {
  ...
  function drawStars() {
    ...
    ctx.fillStyle = "white"; // Set once, use globalAlpha for opacity

    const now = Date.now(); // Cache time to avoid repeated system calls in loop

    stars.current.forEach(star => {
      ctx.beginPath();
      // Set opacity using globalAlpha instead of parsing rgba strings
