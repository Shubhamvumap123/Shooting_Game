...
function App() {
  ...
  function drawStars() {
    ...
    // Optimization: Set fillStyle once to avoid string parsing in loop
    ctx.fillStyle = "white";
    // Optimization: Cache current time to avoid system calls in loop
    const now = Date.now();

    stars.current.forEach(star => {
      ctx.beginPath();
      // Set color with dynamic alpha
      // Optimization: Use globalAlpha instead of template string for better performance
